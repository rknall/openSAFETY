...
*/
static SOD_t_ACCESS s_AccessUdid[EPLS_cfg_MAX_INSTANCES] SAFE_NO_INIT_SEKTOR;
...
BOOLEAN SNMTS_InitParamUdid(BYTE_B_INSTNUM)
{
  ...
  /* get SOD entry "UDID" */
  s_AccessUdid[B_INSTNUMidx].ps_attr =
      SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_UNIQUE_DEVICE_ID,
                  EPLS_k_SUBIDX_UNIQUE_DEVICE_ID,
                  &(s_AccessUdid[B_INSTNUMidx].dw_hdl),
                  &(s_AccessUdid[B_INSTNUMidx].o_applObj), &s_errRes);

  /* if SOD entry is available */
  if(s_AccessUdid[B_INSTNUMidx].ps_attr == NULL)
  {
                           ...
                           UINT8 b_rxUdidLen)
{
  ...
  /* get Udid from SOD */
  pb_udid = (UINT8 *)SOD_Read(B_INSTNUM_ s_AccessUdid[B_INSTNUMidx].dw_hdl,
                              s_AccessUdid[B_INSTNUMidx].o_applObj,
