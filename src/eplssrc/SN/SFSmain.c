...
BOOLEAN SFS_Init(BYTE_B_INSTNUM)
{
  ...
  SOD_t_ACCESS s_acsScmUdid; /* SOD access structure for the UDID of the SCM */
  SOD_t_ERROR_RESULT s_errRes; /* SOD error result */


  /* get the attribute of the SCM UDID */
  s_acsScmUdid.ps_attr = SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_COMMON_COM_PARAM,
                                     EPLS_k_SUBIDX_UDID_SCM,
                                     &s_acsScmUdid.dw_hdl,
                                     &s_acsScmUdid.o_applObj, &s_errRes);

  /* if the attribute access succeeded */
  if (s_acsScmUdid.ps_attr != NULL)
  {
    /* if before read callback is not registered */
    if (EPLS_IS_BIT_RESET(s_acsScmUdid.ps_attr->w_attr, SOD_k_ATTR_BEF_RD))
    {
      /* get the reference to the UDID of the SCM */
      pb_ScmUdid[B_INSTNUMidx] = (UINT8 *)SOD_Read(B_INSTNUM_
                                                   s_acsScmUdid.dw_hdl,
                                                   s_acsScmUdid.o_applObj,
                                                   SOD_k_NO_OFFSET,
