...
*/
static SOD_t_ACCESS s_AcsOwnSdn[EPLS_cfg_MAX_INSTANCES] SAFE_NO_INIT_SEKTOR;
...
BOOLEAN SDN_SetSdn(BYTE_B_INSTNUM_ UINT16 w_ownSdn)
{
  ...
  return SOD_Write(B_INSTNUM_ s_AcsOwnSdn[B_INSTNUMidx].dw_hdl,
                              s_AcsOwnSdn[B_INSTNUMidx].o_applObj, &w_ownSdn,
                              SOD_k_OVERWRITE, SOD_k_NO_OFFSET,
...
static UINT16 * getCheckSdn(BYTE_B_INSTNUM)
{
  ...
  SOD_t_ACCESS *ps_acsOwnSdn = &s_AcsOwnSdn[B_INSTNUMidx]; /* pointer to the
                                                 own SDN SOD access structure */

  /* initiate SOD access data structure */
  ps_acsOwnSdn->ps_attr = (SOD_t_ATTR *)NULL;
  ps_acsOwnSdn->dw_hdl = 0xFFFFFFFFUL;
  ps_acsOwnSdn->o_applObj = FALSE;

  /* get handle to SOD entry: SDN */
  ps_acsOwnSdn->ps_attr = SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_COMMON_COM_PARAM,
                                    EPLS_k_SUBIDX_SDN, &(ps_acsOwnSdn->dw_hdl),
                                    &(ps_acsOwnSdn->o_applObj),
                                    &s_errRes);

  /* if SOD access error happened */
  if(ps_acsOwnSdn->ps_attr == NULL)
  {
    SERR_SetError(B_INSTNUM_ s_errRes.w_errorCode,
                  (UINT32)(s_errRes.e_abortCode));
  }
  /* else if attribute <SOD_k_ATTR_BEF_RD> is set */
  else if(EPLS_IS_BIT_SET(ps_acsOwnSdn->ps_attr->w_attr, SOD_k_ATTR_BEF_RD))
  {
  ...
  {
    /* get SDN from SOD */
    pw_sdn = (UINT16 *) SOD_Read(B_INSTNUM_ ps_acsOwnSdn->dw_hdl,
                                 ps_acsOwnSdn->o_applObj, SOD_k_NO_OFFSET,
