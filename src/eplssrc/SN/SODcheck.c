...
BOOLEAN SOD_Check(BYTE_B_INSTNUM)
{
  ...
  UINT16 w_tmpIdx; /* temporary variable to store SOD index to be checked */
  UINT8  b_tmpSubIdx; /* temp. variable to store SOD sub-index to be checked */
  ...
  while ((ps_tmpSodAddr->w_index != SOD_k_END_OF_THE_OD) && !o_break)
  {
    ...
    {
      w_tmpIdx = ps_tmpSodAddr->w_index;
      b_tmpSubIdx = ps_tmpSodAddr->b_subIndex;

      /* step to the next object in the object dictionary */
      ps_tmpSodAddr++;

      /* if the next object has a smaller index */
      if (w_tmpIdx > ps_tmpSodAddr->w_index)
      {
      ...
      else /* the next object has a large or the same index */
      {
        /* if the next object has the same index and same or smaller sub-idx */
        if ((w_tmpIdx == ps_tmpSodAddr->w_index) &&
            (b_tmpSubIdx >= ps_tmpSodAddr->b_subIndex))
        {
        ...
        /* no else : SOD is sorted */
      }
    }
  }

  /* if the end of the SOD is reached and no error has been found */
  if ((ps_tmpSodAddr->w_index == SOD_k_END_OF_THE_OD) && (!o_break))
  {
    /* SOD is OK */
    o_ret = TRUE;
  }
