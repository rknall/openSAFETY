...
BOOLEAN SSDOS_Init(BYTE_B_INSTNUM)
{
  ...
  SOD_t_ACCESS s_acsMainSadr;  /* handle to refer to main SADR */
  SOD_t_ERROR_RESULT s_errRes; /* SOD access error */

  /* get SOD entry "SN main SADR" */
  s_acsMainSadr.ps_attr = SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_TXSPDO_COM_PARAM,
                                      EPLS_k_SUBIDX_TXSPDO_SADR,
                                      &(s_acsMainSadr.dw_hdl),
                                      &(s_acsMainSadr.o_applObj),
                                      &s_errRes);

  /* if attribute is available */
  if(s_acsMainSadr.ps_attr != NULL)
  {
    /* get main SADR of SN from SOD */
    ps_fsm->pw_mainSadr = (UINT16 *)SOD_Read(B_INSTNUM_ s_acsMainSadr.dw_hdl,
                                             s_acsMainSadr.o_applObj,
                                             SOD_k_NO_OFFSET,
                                 ...
                                 EPLS_t_FRM *ps_txBuf, UINT32 dw_abortCode)
{
  ...
  SSDOS_SodAcsResp(B_INSTNUM_ ps_rxBuf, ps_txBuf, k_ABORT,
