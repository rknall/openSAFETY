...
static BOOLEAN InitTxSpdo(BYTE_B_INSTNUM_ UINT16 *pw_noTxSpdo)
{
  ...
  *pw_noTxSpdo = 0U;
  ...
  do
  {
    ...
    if (SOD_AttrGet(B_INSTNUM_ w_commIdx, 0U, &dw_sodHdl, &o_appObj,
                    &s_errRes) != NULL)
    {
    ...
    else
    {
      ...
      #endif
      {
        ...
        /* if SPDO mapping parameter does not exist for this SPDO comm.
              parameter */
        if (SOD_AttrGet(B_INSTNUM_ w_mappIdx, 0U, &dw_sodHdl, &o_appObj,
                        &s_errRes) == NULL)
        {
          #if (SOD_cfg_APPLICATION_OBJ == EPLS_k_DISABLE)
            #pragma CTC SKIP
                              ...
                              UINT16 w_txCommIdx, UINT16 w_txMappIdx)
{
  ...
  UINT32 dw_sodHdl; /* handle for SOD access */
  BOOLEAN o_appObj; /* application object flag for SOD access */
  const SOD_t_ATTR *ps_attr; /* pointer to the attribute of the SOD object */
  SOD_t_ERROR_RESULT s_errRes; /* SOD access error */
  ...
  {
    /* get attribute of the number of entries */
    ps_attr = SOD_AttrGet(B_INSTNUM_ w_txMappIdx, 0U, &dw_sodHdl, &o_appObj,
                          &s_errRes);
    ...
    else if (EPLS_IS_BIT_RESET(ps_attr->w_attr, SOD_k_ATTR_BEF_WR))
    {
  ...
  */
                                ...
                                UINT16 w_rxCommIdx, UINT16 w_rxMappIdx)
  {
    ...
    {
      /* get attribute of the number of entries */
      ps_attr = SOD_AttrGet(B_INSTNUM_ w_rxMappIdx, 0U ,&dw_sodHdl, &o_appObj,
                            &s_errRes);
