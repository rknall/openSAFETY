 ...
 */
static SOD_t_ACCESS s_AcsMainSadr[EPLS_cfg_MAX_INSTANCES] SAFE_NO_INIT_SEKTOR;

/**
 * @var s_AcsUdidScm
 * Data structure to manage access to the SOD entry 0x1200, 0x04, which contains the "UDID of the SCM".
 */
static SOD_t_ACCESS s_AcsUdidScm[EPLS_cfg_MAX_INSTANCES] SAFE_NO_INIT_SEKTOR;
...
{
    SCFM_TACK_PATH();
    return SOD_Write(B_INSTNUM_ s_AcsMainSadr[B_INSTNUMidx].dw_hdl,
            s_AcsMainSadr[B_INSTNUMidx].o_applObj, &w_mainSadr,
            SOD_k_NO_OVERWRITE, SOD_k_NO_OFFSET, SOD_k_LEN_NOT_NEEDED);
}
...
{
    SCFM_TACK_PATH();
    return SOD_Write(B_INSTNUM_ s_AcsUdidScm[B_INSTNUMidx].dw_hdl,
            s_AcsUdidScm[B_INSTNUMidx].o_applObj, pb_udidScm,
            SOD_k_NO_OVERWRITE, SOD_k_NO_OFFSET, SOD_k_LEN_NOT_NEEDED);
}
...
void SNMTS_TxRespUdid(BYTE_B_INSTNUM_ const EPLS_t_FRM *ps_rxBuf,
        EPLS_t_FRM *ps_txBuf, const SOD_t_ACCESS *ps_accessUdid) 
{
...
static BOOLEAN MainSadrAcsInit(BYTE_B_INSTNUM) 
{
    ...
    s_AcsMainSadr[B_INSTNUMidx].ps_attr =
        SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_TXSPDO_COM_PARAM, 
                               EPLS_k_SUBIDX_TXSPDO_SADR,
                               &(s_AcsMainSadr[B_INSTNUMidx].dw_hdl),
                               &(s_AcsMainSadr[B_INSTNUMidx].o_applObj),
                               &s_errRes);

    /* if SOD access failed */
    if (s_AcsMainSadr[B_INSTNUMidx].ps_attr == NULL ) 
	{
        SERR_SetError(B_INSTNUM_ s_errRes.w_errorCode,
                (UINT32) (s_errRes.e_abortCode));
    }
    else  /* SOD entry "SN main SADR" available */
    {
        apw_MainSadr[B_INSTNUMidx] =
            (UINT16 *)SOD_Read(B_INSTNUM_ s_AcsMainSadr[B_INSTNUMidx].dw_hdl,
                                      s_AcsMainSadr[B_INSTNUMidx].o_applObj,
...
static BOOLEAN UdidScmAcsInit(BYTE_B_INSTNUM) 
{
    ...
    s_AcsUdidScm[B_INSTNUMidx].ps_attr =
        SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_COMMON_COM_PARAM,
                               EPLS_k_SUBIDX_UDID_SCM,
                               &(s_AcsUdidScm[B_INSTNUMidx].dw_hdl),
                               &(s_AcsUdidScm[B_INSTNUMidx].o_applObj),
                               &s_errRes);

    /* if the SOD access failed */
    if (s_AcsUdidScm[B_INSTNUMidx].ps_attr == NULL ) 
	{
        SERR_SetError(B_INSTNUM_ s_errRes.w_errorCode,
                (UINT32) (s_errRes.e_abortCode));
    }
