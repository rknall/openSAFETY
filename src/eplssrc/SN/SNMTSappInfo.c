...
*/
static SOD_t_ACCESS s_AccessParamTs[EPLS_cfg_MAX_INSTANCES] SAFE_NO_INIT_SEKTOR;
...
BOOLEAN SNMTS_AppInfoInit(BYTE_B_INSTNUM)
{
  ...
  s_AccessParamTs[B_INSTNUMidx].ps_attr =
    SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_DEVICE_VEN_ID, EPLS_k_SUBIDX_PARAM_TS,
      &(s_AccessParamTs[B_INSTNUMidx].dw_hdl),
      &(s_AccessParamTs[B_INSTNUMidx].o_applObj), &s_errRes);

  /* if the "parameter timestamp" attribute is available */
  if(s_AccessParamTs[B_INSTNUMidx].ps_attr != NULL)
  {
    o_return = TRUE;
  }
...
static BOOLEAN getParamTs(BYTE_B_INSTNUM_ UINT32 *pdw_paramTs)
{
  ...
  pdw_help =  (UINT32 *)SOD_Read(B_INSTNUM_
                                 s_AccessParamTs[B_INSTNUMidx].dw_hdl,
                                 s_AccessParamTs[B_INSTNUMidx].o_applObj,
