                                ...
                                UINT16 w_index)
{
  ...
  {
    SPDO_ConsSmInit(B_INSTNUM_ w_rxSpdoIdx, (UINT16 *)pv_sct,
 ...
 *
 * @retval       pb_noFreeFrm            number of the free frames to be sent (pointer not checked,
...
{
  BOOLEAN o_ret = FALSE; /* return value */
  const SOD_t_ATTR *ps_sodAttr; /* pointer to the attribute of the SOD object */
  ...
  ps_sodAttr = SOD_AttrGet(B_INSTNUM_ w_index, b_subIdx, &dw_sodHdl, &o_appObj,
                           &s_errRes);
    ...
    if (pv_data != NULL)
    {
      switch (b_subIdx)
      {
        ...
        case k_SIDX_TIME_REQUEST_CYCLE:
        {
          ...
          if (EPLS_IS_BIT_SET(ps_sodAttr->w_attr, SOD_k_ATTR_BEF_RD))
          {
...
*
* @retval       pb_noFreeFrm            number of the free frames to be sent (pointer not checked, checked in
                             ...
                             UINT8 *pb_noFreeFrm)
{
  ...
  if (ps_rxSpdoHeader != NULL)
  {
                  ...
                  ps_rxSpdo->s_rxCtr.b_cntTr))
    {
      ...
      /* if the time synchronization propagation delay is OK */
      if (( *ps_rxSpdo->s_rxCommPara.pw_minTSyncPropDelay <= dw_propDelay ) &&
          ( *ps_rxSpdo->s_rxCommPara.pw_maxTSyncPropDelay >= dw_propDelay ))
      {
      ...
      /* else if the time synchronization propagation delay is too short */
      else if ( *ps_rxSpdo->s_rxCommPara.pw_minTSyncPropDelay > dw_propDelay )
      {
...
static void IncTReqCounters(UINT32 dw_ct, t_RX_SYNC_CONS_SPDO *ps_rxSpdo)
{
      ...
      *ps_rxSpdo->s_rxCommPara.pb_noConsecutiveTReq)
  {
    ...
    ps_rxSpdo->s_timers.dw_maxProp = (UINT32)(dw_ct +
        *ps_rxSpdo->s_rxCommPara.pw_maxTSyncPropDelay);
  }
