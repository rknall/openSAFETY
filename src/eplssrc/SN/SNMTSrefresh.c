...
*/
static SOD_t_ACCESS as_AccessRefreshTime[EPLS_cfg_MAX_INSTANCES]
                      SAFE_NO_INIT_SEKTOR;
/**
 * @var as_AccessRetries
 * SOD access structure array to get the refresh time from the SOD.
*/
static SOD_t_ACCESS as_AccessRetries[EPLS_cfg_MAX_INSTANCES]
                      SAFE_NO_INIT_SEKTOR;
...
BOOLEAN SNMTS_RefreshTimeInit(BYTE_B_INSTNUM)
{
  ...
  as_AccessRefreshTime[B_INSTNUMidx].ps_attr =
    SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_PRE_OP_SIGNAL, EPLS_k_SUBIDX_REFRESH_TIME,
      &(as_AccessRefreshTime[B_INSTNUMidx].dw_hdl),
      &(as_AccessRefreshTime[B_INSTNUMidx].o_applObj), &s_errRes);

  /* if access to the SOD entry NOT possible */
  if(as_AccessRefreshTime[B_INSTNUMidx].ps_attr == NULL)
  {
  ...
  else  /* SOD entry "Refresh Time" available */
  {
	  ...
	  as_AccessRetries[B_INSTNUMidx].ps_attr =
	    SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_PRE_OP_RETRIES, EPLS_k_SUBIDX_NO_RETRIES,
	      &(as_AccessRetries[B_INSTNUMidx].dw_hdl),
	      &(as_AccessRetries[B_INSTNUMidx].o_applObj), &s_errRes);

	  /* if access to the SOD entry NOT possible */
	  if(as_AccessRetries[B_INSTNUMidx].ps_attr == NULL)
	  {
		SERR_SetError(B_INSTNUM_ s_errRes.w_errorCode,
					  (UINT32)(s_errRes.e_abortCode));
	  }
...
static BOOLEAN SNMTS_RefreshTimeUpdate(BYTE_B_INSTNUM_ UINT32 dw_ct)
{
  ...
  pdw_refreshInterval = (UINT32 *)SOD_Read(B_INSTNUM_
                                   as_AccessRefreshTime[B_INSTNUMidx].dw_hdl,
                                   as_AccessRefreshTime[B_INSTNUMidx].o_applObj,
                               ...
                               BOOLEAN* po_elapsed)
{
  ...
  /* get Refresh Time from SOD */
  pucMaxRetries = (UINT8 *)SOD_Read(B_INSTNUM_
		  as_AccessRetries[B_INSTNUMidx].dw_hdl,
		  as_AccessRetries[B_INSTNUMidx].o_applObj,
          SOD_k_NO_OFFSET, SOD_k_LEN_NOT_NEEDED,
          &s_errRes);
