...
*/
static SOD_t_ACCESS as_AccessGuardingTime[EPLS_cfg_MAX_INSTANCES]
                      SAFE_NO_INIT_SEKTOR;
...
*/
static SOD_t_ACCESS as_AccessLifeTime[EPLS_cfg_MAX_INSTANCES]
                      SAFE_NO_INIT_SEKTOR;
...
BOOLEAN SNMTS_LifeTimeInit(BYTE_B_INSTNUM)
{
  ...
  as_AccessGuardingTime[B_INSTNUMidx].ps_attr =
    SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_LIFE_GUARDING, EPLS_k_SUBIDX_GUARD_TIME,
      &(as_AccessGuardingTime[B_INSTNUMidx].dw_hdl),
      &(as_AccessGuardingTime[B_INSTNUMidx].o_applObj), &s_errRes);

  /* if SOD entry is not available */
  if(as_AccessGuardingTime[B_INSTNUMidx].ps_attr == NULL)
  {
  ...
  else  /* SOD entry "Guard Time" available */
  {
    ...
    as_AccessLifeTime[B_INSTNUMidx].ps_attr =
      SOD_AttrGet(B_INSTNUM_ EPLS_k_IDX_LIFE_GUARDING,
        EPLS_k_SUBIDX_LIFE_TIME_FACTOR,
        &(as_AccessLifeTime[B_INSTNUMidx].dw_hdl),
        &(as_AccessLifeTime[B_INSTNUMidx].o_applObj), &s_errRes);

    /* if SOD entry is not available */
    if(as_AccessLifeTime[B_INSTNUMidx].ps_attr == NULL)
    {
      SERR_SetError(B_INSTNUM_ s_errRes.w_errorCode,
                    (UINT32)(s_errRes.e_abortCode));
    }
...
BOOLEAN SNMTS_LifeTimeStart(BYTE_B_INSTNUM_ UINT32 dw_ct)
{
  ...
  if (guardTimeStart(B_INSTNUM_ dw_ct))
  {
    ...
    pb_lifeTimeFactor =
       (UINT8 *)SOD_Read(B_INSTNUM_ as_AccessLifeTime[B_INSTNUMidx].dw_hdl,
                         as_AccessLifeTime[B_INSTNUMidx].o_applObj,
                         SOD_k_NO_OFFSET, SOD_k_LEN_NOT_NEEDED, &s_errRes);
...
static BOOLEAN guardTimeStart(BYTE_B_INSTNUM_ UINT32 dw_ct)
{
  ...
  pdw_guardInterval =
            (UINT32 *)SOD_Read(B_INSTNUM_
                               as_AccessGuardingTime[B_INSTNUMidx].dw_hdl,
                               as_AccessGuardingTime[B_INSTNUMidx].o_applObj,
