...
*
* If a TxSPDO was built and sent then pb_noFreeFrm will be decremented.
*
...
* @param        dw_ct                   consecutive time, internal timer value (not checked, any value allowed) valid range: (UINT32)
*
* @retval       pb_noFreeFrm            reference to the number of the free SPDO frames to be sent (checked) valid range: <> NULL
*/
void SPDO_BuildTxSpdo(BYTE_B_INSTNUM_ UINT32 dw_ct, UINT8 *pb_noFreeFrm)
{
  t_SPDO_OBJ *po_this;    /* instance pointer */
      ...
      if (po_this->o_spdoRunning)
      {
        ...
        for(w_spdoIdx = 0U; w_spdoIdx < w_noProcTxSpdo; w_spdoIdx++)
        {
          ...
          SPDO_TimeSyncProdSm(B_INSTNUM_ dw_ct, w_spdoIdx,
                              (EPLS_t_FRM_HDR *)NULL, pb_noFreeFrm);
        }
        ...
        for(w_spdoIdx = 0U; w_spdoIdx < w_noProcTxSpdo; w_spdoIdx++)
        {
          ...
          SPDO_ProdSm(B_INSTNUM_ dw_ct, w_spdoIdx, pb_noFreeFrm);
        }
      }
      /* no else : the SPDO is not active */
    }
...
UINT8 SPDO_BuildTxSpdoIx(BYTE_B_INSTNUM_ UINT32 dw_ct, UINT16 w_spdoIdx)
{
  ...
  UINT8 b_noFreeFrm=	1;		/* 1 TxSPDO shall be generated	*/
    ...
    else /* all parameter ok */
    {
      ...
      if (po_this->o_spdoRunning)
      {
        /* Check if Time Response SPDO is to be sent */
        SPDO_TimeSyncProdSm(B_INSTNUM_ dw_ct, w_spdoIdx,
                              (EPLS_t_FRM_HDR *)NULL, &b_noFreeFrm);
        ...
        {
          /* Check if Data only SPDO is to be sent */
          SPDO_ProdSm(B_INSTNUM_ dw_ct, w_spdoIdx, &b_noFreeFrm);
        }
      } /* if spdoRunning */
      /* no else : the SPDO is not active */
    }
...
BOOLEAN SPDO_ActivateTxSpdoMapping(BYTE_B_INSTNUM_ BOOLEAN *po_busy)
{
  ...
  t_SPDO_OBJ *ps_spdoObj = &as_Obj[B_INSTNUMidx]; /* pointer the SPDO object
                                                     structure */
  UINT32 dw_sodHdl;            /* handle for SOD access */
  BOOLEAN o_appObj;            /* application object flag for SOD access */
  ...
  *po_busy = FALSE;
  ...
  if (ps_spdoObj->w_actTxSpdoIdx < ps_spdoObj->w_noTxSpdo)
  {
    do
    {
      ...
      if (SOD_AttrGet(B_INSTNUM_ (UINT16)(k_TX_MAPP_START_IDX +
                                          ps_spdoObj->w_actTxSpdoIdx),
                      0U, &dw_sodHdl, &o_appObj, &s_errRes) != NULL)
      {
  ...
  BOOLEAN o_ret = FALSE;         /* return value */
    ...
    *po_busy = FALSE;
    ...
    if (ps_spdoObj->w_actRxSpdoIdx < ps_spdoObj->w_noRxSpdo)
    {
      ...
      {
        /* if attribute of the mapping parameter sub 0 was read successful */
        if (SOD_AttrGet(B_INSTNUM_ (UINT16)(k_RX_MAPP_START_IDX +
          ...
          if (pv_data != NULL)
          {
              ...
              SPDO_CheckRxCommPara(B_INSTNUM_ ps_spdoObj->w_actRxSpdoIdx))
            {
              ...
              o_ret = SPDO_RxMappActivate(B_INSTNUM_ ps_spdoObj->w_actRxSpdoIdx,
                                          *((UINT8*)(pv_data)));
#pragma CTC SKIP
              #if (SPDO_cfg_MAX_SYNC_RX_SPDO==1)	/* remember the RxSPDO that is connected to this TxSPDO	*/
              {
                uint8_t	txspdo=	SPDO_GetTxSpdoNo(B_INSTNUM_ ps_spdoObj->w_actRxSpdoIdx) -1;
                if ( (txspdo < ps_spdoObj->w_noTxSpdo) && (ps_spdoObj->w_actRxSpdoIdx < ps_spdoObj->w_noRxSpdo) )
                {  
                  ps_spdoObj->aw_TxRxConn[txspdo]=	ps_spdoObj->w_actRxSpdoIdx;
                }
              }
	      #endif
#pragma CTC ENDSKIP
