...
*/
SOD_t_ACCESS SNMTM_s_AccessSadrScm SAFE_NO_INIT_SEKTOR;
...
BOOLEAN SNMTM_Init(void)
{
  ...
  /* get handle to the SOD entry: SADR of SCM */
  SNMTM_s_AccessSadrScm.ps_attr =
    SOD_AttrGet(EPLS_k_SCM_INST_NUM_ EPLS_k_IDX_COMMON_COM_PARAM,
      EPLS_k_SUBIDX_SADR_SCM, &(SNMTM_s_AccessSadrScm.dw_hdl),
      &(SNMTM_s_AccessSadrScm.o_applObj), &s_errRes);

  /* if write access to the SOD entry NOT possible */
  if(SNMTM_s_AccessSadrScm.ps_attr == NULL)
  {
...
static BOOLEAN IsDsadrScmSadr(UINT16 w_rxDsadr)
{
  ...
  pw_scmSadr =
    (UINT16 *)SOD_Read(EPLS_k_SCM_INST_NUM_ SNMTM_s_AccessSadrScm.dw_hdl,
                       SNMTM_s_AccessSadrScm.o_applObj, SOD_k_NO_OFFSET,
