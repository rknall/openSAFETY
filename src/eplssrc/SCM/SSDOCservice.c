                                ...
                                UINT8 b_respTr)
{
  ...
  if(as_ServFsm[w_fsmNum].o_waitForReq)
  {
    ...
    else if(e_fsmEvent == k_EVT_ABORT_SEND)
    {
      ...
      if(!reqBufferSend(w_fsmNum))
      {
        /* FATAL error is already reported */
        serviceFsmInit(w_fsmNum);
      }
      else /* transmission of the service abort request succeeded */
      {
        o_return = TRUE;
      }
    }
                               ...
                               UINT8 b_respTr)
{
  ...
  if(b_respTr != EPLS_k_TR_NOT_USED)
  {
    SERR_SetError(EPLS_k_SCM_INST_NUM_ SSDOC_k_ERR_RESP_TR_FIELD_USED,
                  (UINT32)b_respTr);
  }
  /* else if the echo of the SOD access req number (SANo) is wrong */
  else if(w_respCt != as_ServFsm[w_fsmNum].s_reqBuf.s_hdr.w_ct)
  {
