...
static SOD_t_ACCESS s_AccessScmSadr SAFE_NO_INIT_SEKTOR;
...
BOOLEAN SSDOC_ServiceLayerInit(void)
{
  ...
  s_AccessScmSadr.ps_attr =
    SOD_AttrGet(EPLS_k_SCM_INST_NUM_ EPLS_k_IDX_COMMON_COM_PARAM,
                EPLS_k_SUBIDX_SADR_SCM, &(s_AccessScmSadr.dw_hdl),
                &(s_AccessScmSadr.o_applObj), &s_errRes);

  /* if write access to the SOD entry NOT possible */
  if(s_AccessScmSadr.ps_attr == NULL)
  {
...
static BOOLEAN getScmSadrSdn(UINT16 *pw_scmSadr)
{
  ...
  pw_tmpScmSadr = (UINT16 *)SOD_Read(EPLS_k_SCM_INST_NUM_
                                     s_AccessScmSadr.dw_hdl,
                                     s_AccessScmSadr.o_applObj,
