 ...
 * extended services.
*/
#define k_MAX_RESP_EXT_SERV_FRM_TYPE   0x0Fu
...
BOOLEAN SNMTM_InitFsm(void)
{
  ...
  /* get handle to SOD entry of SNMT Master response timeout */
  s_AccessTimeout.ps_attr =
    SOD_AttrGet(EPLS_k_SCM_INST_NUM_ EPLS_k_IDX_SNMT_COM_PARAM,
                EPLS_k_SUBIDX_SNMT_TIMEOUT, &(s_AccessTimeout.dw_hdl),
                &(s_AccessTimeout.o_applObj), &s_errRes);
  ...
  {
    /* get handle to SOD entry of SNMT Master response retries */
    s_AccessRetries.ps_attr =
      SOD_AttrGet(EPLS_k_SCM_INST_NUM_ EPLS_k_IDX_SNMT_COM_PARAM,
        EPLS_k_SUBIDX_SNMT_RETRIES, &(s_AccessRetries.dw_hdl),
  ...
  if(EPLS_TIMEOUT(dw_ct, as_Fsm[w_fsmNum].dw_respTimeout))
  {
       ...
       as_Fsm[w_fsmNum].b_nomNumOfRep)
    {
      ...
      SERR_SetError(EPLS_k_SCM_INST_NUM_ SNMTM_k_ERR_NO_RESP_RECEIVED,
                    (UINT32)(as_Fsm[w_fsmNum].w_regNum));
...
static BOOLEAN transmitRequest(UINT16 w_fsmNum, UINT32 dw_ct)
{
  ...
  else  /* memory allocation succeeded */
  {
    ...
    pdw_respTimeout =
      (UINT32 *)SOD_Read(EPLS_k_SCM_INST_NUM_ s_AccessTimeout.dw_hdl,
                         s_AccessTimeout.o_applObj, SOD_k_NO_OFFSET,
    ...
    else  /* response timeout available */
    {
      ...
      /* get response timeout value from SOD */
      pb_reqRetries =
        (UINT8 *)SOD_Read(EPLS_k_SCM_INST_NUM_ s_AccessRetries.dw_hdl,
                          s_AccessRetries.o_applObj, SOD_k_NO_OFFSET,
                          SOD_k_LEN_NOT_NEEDED, &s_errRes);
