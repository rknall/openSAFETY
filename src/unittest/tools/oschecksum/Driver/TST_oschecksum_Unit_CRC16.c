...
void TST_CRC16_cksum_protocol_prec(void)
{
    ...
    {
        count[0] = (uint8_t) ctr;
        CU_ASSERT_EQUAL_FATAL(ui_openSAFETY_CRC16(1, CRC16_POLY, count),
                crc16Checksum(1, count, 0));
    }
}
...
void TST_CRC16_rnd_subframe(void)
{
    ...
    CU_ASSERT_EQUAL_FATAL(ui_openSAFETY_CRC16(c_sub_length1, CRC16_POLY, uc_subframe_1),
                    crc16ChecksumCalculator(c_sub_length1, uc_subframe_1, 0));

    CU_ASSERT_EQUAL_FATAL(ui_openSAFETY_CRC16(c_sub_length2, CRC16_POLY, uc_subframe_2),
